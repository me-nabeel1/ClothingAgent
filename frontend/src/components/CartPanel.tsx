...
export function CartPanel({ cart, open, disabled, onClose, onAction }: CartPanelProps) {
  ...
  return (
    <aside className={`cart-panel ${open ? "cart-panel--open" : ""}`} aria-label="Shopping cart">
                    ...
                    <div className="cart-item__controls">
                      <button
                        ...
                        onClick={() => item.quantity === 1
                          ? onAction(`Remove item ${index + 1} from cart`)
                          : onAction(`Update item ${index + 1} quantity to ${item.quantity - 1}`)}
                      >
                        <Minus size={14} />
                      </button>
                      ...
                      <button
                        ...
                        onClick={() => onAction(`Update item ${index + 1} quantity to ${item.quantity + 1}`)}
                      >
                        <Plus size={14} />
                      </button>
                      <button
                        ...
                        disabled={disabled}
                        onClick={() => onAction(`Remove item ${index + 1} from cart`)}
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
      ...
      <div className="cart-panel__footer">
        ...
        <p>Checkout is outside this demo. The cart uses live local inventory and prices.</p>
        {items.length > 0 && (
          <button type="button" className="button button--danger" disabled={disabled} onClick={() => onAction("Clear my cart")}>Clear bag</button>
        )}
      </div>
    </aside>
  );
