...
class CatalogService:
    ...
    async def search(self, request: ProductSearchRequest) -> ProductSearchResponse:
        ...
        offers = []
        if self._promotions:
            offers = await self._promotions.repository.get_active_offers()
    ...
    def _group_rows_into_products(self, rows: list[dict[str, Any]], offers: list[Any] | None = None) -> list[ProductView]:
        ...
        for r in rows:
            ...
            if vid not in p_map["variants_map"]:
                ...
                # Use the same deterministic promotion rules used by checkout.
                promotion = None
                if self._promotions:
                    promotion = self._promotions.best_variant_offer(
                        offers,
                        product_id=pid,
                        variant_id=vid,
                ...
                if promotion is not None:
                    best_discount = promotion.discount_total
                    applied_offer = promotion.applied_offers[0] if promotion.applied_offers else None
                else:
                    best_discount = Decimal("0.00")
                    applied_offer = None
                final_price = max(Decimal("0.00"), base_price - best_discount)
    ...
    async def get_store_context(self) -> StoreContext:
        ...
        return StoreContext(
            store_name=f"{get_config().store_id.title()} Store",
