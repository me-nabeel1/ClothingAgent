...
class CatalogRepository:
    ...
    async def list_branches(self) -> list[dict[str, Any]]:
        ...
        result = await self._db.execute(
            ...
            .where(Branch.is_active.is_(True), Branch.store_id == get_config().store_id)
    ...
    async def get_distinct_values(self) -> dict[str, list[str]]:
        ...
        categories = (await self._db.scalars(select(Category.category_name).where(Category.store_id == get_config().store_id).distinct())).all()
        product_types = (await self._db.scalars(select(Product.product_type).where(Product.store_id == get_config().store_id).distinct())).all()
        occasions = (await self._db.scalars(select(Product.occasion).where(Product.store_id == get_config().store_id, Product.occasion != None).distinct())).all()
        ...
        seasons = (await self._db.scalars(select(Product.season).where(Product.store_id == get_config().store_id, Product.season != None).distinct())).all()
    ...
    def _base_statement(cls) -> Select:
        ...
        return (
            ...
            .where(
                ...
                Category.store_id == get_config().store_id,
            )
        )
