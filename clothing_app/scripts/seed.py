...
from app.catalog.models import Store, Branch, Category, Product, ProductVariant, Color, Size, ProductImage
from app.inventory.models import BranchInventory
from app.promotions.models import Offer

async def seed_db(db: AsyncSession):
    # Rebuild the demo dataset through one canonical seed path.
    await db.execute(text("TRUNCATE TABLE clothing_store.inventory_reservations, clothing_store.order_items, clothing_store.orders, clothing_store.cart_items, clothing_store.carts, clothing_store.branch_inventory, clothing_store.offers, clothing_store.product_images, clothing_store.product_variants, clothing_store.products, clothing_store.categories, clothing_store.branches, clothing_store.colors, clothing_store.sizes CASCADE"))
    await db.execute(text("DELETE FROM clothing_store.stores"))
    db.add(Store(store_id="northstar", store_name="Northstar Menswear", is_active=True))
    await db.flush()
