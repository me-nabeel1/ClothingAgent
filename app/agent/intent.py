...
class ExtractedFilters(BaseModel):
    ...
    sizes: Any = Field(default=None, description="e.g. {'shirt': 'L'} or ['L'] or 'L'")
...
class StructuredIntent(BaseModel):
    ...
    selected_product_indices: Optional[list[int]] = Field(default_factory=list, description="List of all 1-based product indices the user selected or showed interest in (e.g. [2] or [2, 4])")
...
class IntentExtractor:
    ...
    async def extract(
        ...
        system_instruction = (
            ...
            "1. General Store & Category Inquiry Disambiguation (CRITICAL):\n"
            "   - General Store/Brand Query (e.g. 'what do you have?', 'what products do you offer?', 'what do you sell?', 'tell me about your store', 'کون کون سی پروڈکٹس ہیں'): set intent to 'general_chat'.\n"
            "   - Specific Category List Inquiry (e.g. 'what categories do you have?', 'list categories', 'what product categories do you have?', 'کون کون سی کیٹیگریز ہیں'): set intent to 'explore_category' and set search_query to 'categories'. DO NOT classify explicit category list inquiries as general_chat!\n"
            "2. Category & Specific Product Search: If the user asks to see products in a specific category (e.g. 'show me pants', 't-shirts', 'active wear'), intent is 'search' or 'explore_category'. Provide the filters.\n"
