...
class ConversationState(BaseModel):
    ...
    def sync_displayed_products_with_reply(self, reply: str) -> None:
        """Sync displayed_products and product_cards so cards are ONLY displayed during search/discovery turns for products described in reply text. Suppress cards during detail queries, follow-up turns, and variant requests."""
        ...
        # Product cards are displayed ONLY during search / discovery intents ('search', 'explore_category')
        if self.current_intent in ["search", "explore_category"] and matched_products:
